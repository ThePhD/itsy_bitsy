...
namespace __gnu_cxx _GLIBCXX_VISIBILITY(default)
{
	...
	public:
		...
		: _M_bounds(), _M_storage(::std::forward<_Arg>(__arg), ::std::forward<_Args>(__args)...)
		{
		}

		template<typename... _Args>
		__bit_view(bounds_type __bounds, _Args&&... __args) noexcept(
		  ::std::is_nothrow_constructible_v<container_type,
		    _Args...>&& ::std::is_nothrow_move_constructible_v<bounds_type>)
		: _M_bounds(std::move(__bounds)), _M_storage(::std::forward<_Args>(__args)...)
		{
		}
		...
		begin() noexcept
		{
			if constexpr (__is_word_bit_bounds_v<bounds_type>)
				{
					return iterator(this->_M_storage_begin(), 0);
				}
			else
				{
					auto __bit_distance = this->_M_bounds.begin_position(this->_M_storage_unwrapped());
		...
		end() noexcept
		{
			if constexpr (__is_word_bit_bounds_v<bounds_type>)
				...
				{
					auto __bit_distance = this->_M_bounds.end_position(this->_M_storage_unwrapped());
		...
		empty() const
		{
			if constexpr (__is_word_bit_bounds_v<bounds_type>)
				...
				{
					return this->_M_bounds.begin_position(this->_M_storage_unwrapped()) ==
					       this->_M_bounds.end_position(this->_M_storage_unwrapped());
				}
		}
		...
		size() const noexcept
		{
			return this->_M_bounds.end_position(this->_M_storage_unwrapped()) -
			       this->_M_bounds.begin_position(this->_M_storage_unwrapped());
		}
	...
	private:
		...
		inline static constexpr bool __is_directly_comparable =
		  ::std::is_unsigned_v<__integral_word_type>&& ::std::is_unsigned_v<
		    ...
		    typename _Right::bounds_type>&& __is_word_bit_bounds_v<bounds_type>&&
		    __is_word_bit_bounds_v<typename _Right::bounds_type>;

		constexpr size_type
		_M_bit_distance() const
		{
			return static_cast<size_type>(this->_M_bounds.end_position(this->_M_storage_unwrapped()) -
			                              this->_M_bounds.begin_position(this->_M_storage_unwrapped()));
		}
		...
		if constexpr (_Left::template __is_directly_comparable<_Right>)
		...
		else
			{
				...
				if constexpr (std::is_same_v<typename _Left::__word_type, typename _Right::__word_type> &&
				              __is_word_bit_bounds_v<typename _Left::bounds_type> &&
				              __is_word_bit_bounds_v<typename _Right::bounds_type>)
					{
