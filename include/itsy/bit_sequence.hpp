...
#define ITSY_BITSY_BIT_SEQUENCE_HPP 1
...
namespace bitsy
{
	...
	class bit_sequence : private bit_view<_Container, word_bit_bounds<_Container>>
	{
	private:
		...
		using __base_t                   = bit_view<_Container, word_bit_bounds<_Container>>;
		...
		using __range                    = typename __base_t::__range;
	...
	public:
		...
		using iterator_concept  = typename __base_t::iterator_concept;
		...
		// constructors
		bit_sequence() noexcept(noexcept(__base_t())) : __base_t(), _M_bit_pos(binary_digits_v<__base_value_type>)
		{
		}

		template<typename... _Args>
		bit_sequence(::std::in_place_t, _Args&&... __args) noexcept(
		     noexcept(__base_t(::std::forward<_Args>(__args)...)))
		: __base_t(::std::forward<_Args>(__args)...), _M_bit_pos(binary_digits_v<__base_value_type>)
		{
		}
		...
		bit_sequence(_Iterator __first, _Sentinel __last) noexcept(noexcept(bit_sequence(__ib_detail::__dummy_tag{},
		     bit_sequence::_S_efficient_empty_create(::std::move(__first), ::std::move(__last)))))
		: bit_sequence(__ib_detail::__dummy_tag{},
		       bit_sequence::_S_efficient_empty_create(::std::move(__first), ::std::move(__last)))
		{
		}

		bit_sequence(size_type __desired_count) noexcept(
		     noexcept(bit_sequence(__desired_count, static_cast<value_type>(false))))
		: bit_sequence(__desired_count, static_cast<value_type>(false))
		{
		}

		bit_sequence(size_type __desired_count, value_type __val) noexcept(noexcept(bit_sequence(
		     __ib_detail::__dummy_tag{}, bit_sequence::_S_efficient_empty_create_sized(__desired_count, __val))))
		: bit_sequence(
		       __ib_detail::__dummy_tag{}, bit_sequence::_S_efficient_empty_create_sized(__desired_count, __val))
		{
		}

		bit_sequence(::std::initializer_list<value_type> __il) noexcept(
		     noexcept(bit_sequence(__il.begin(), __il.end())))
		: bit_sequence(__il.begin(), __il.end())
		{
		}

		bit_sequence(const bit_sequence& __right) = default;

		bit_sequence(bit_sequence&& __right) = default;

		// assignment
		bit_sequence&
		operator=(const bit_sequence& __right) = default;

		bit_sequence&
		operator=(bit_sequence&& __right) = default;
		...
		bit_sequence&
		assign(const bit_sequence& __right)
		{
			return this->operator=(__right);
		}

		bit_sequence&
		assign(bit_sequence&& __right)
		{
		...
		push_back(value_type __val)
		{
			...
			if (_M_bit_pos == binary_digits_v<__base_value_type>)
				{
			...
			for (; __storage_first != __storage_last; ++__storage_first)
				{
					...
					__bit_ref __back(__ref, __ib_detail::__max_binary_index_v<__base_value_type>);
					__old_val = __back;
					__ref <<= 1;
					__front = __val;
					__val   = __old_val;
		...
		insert(const_iterator __where, value_type __val)
		{
			...
			if (this->_M_bit_pos == binary_digits_v<__base_value_type>)
				{
			...
			value_type __old_val = reference(__first_ref, __ib_detail::__max_binary_index_v<__base_value_type>);

			// are we not at the MSB edge of a word?
			if (__where_pos != __ib_detail::__max_binary_index_v<__base_value_type>)
				{
			...
			for (; __storage_it != __storage_last; ++__storage_it)
				{
					...
					value_type __saved =
					     reference(__storage_current_ref, __ib_detail::__max_binary_index_v<__base_value_type>);
		...
		{
			using _ItConcept = ::ztd::ranges::iterator_concept_t<_Iterator>;
			...
			if constexpr (::ztd::ranges::is_concept_or_better_v<std::random_access_iterator_tag, _ItConcept>)
				{
		...
		pop_back()
		{
			...
			if (this->_M_bit_pos == 0)
				{
					this->_M_bit_pos = binary_digits_v<__base_value_type>;
			...
			for (; __storage_first != __storage_last; ++__storage_first, (void)++__old_storage_it)
				{
					...
					reference __old_bit_ref(
					     *__old_storage_it, __ib_detail::__max_binary_index_v<__base_value_type>);
		...
		erase(const_iterator __first, const_sentinel __last) noexcept
		{
			...
			if constexpr (::ztd::ranges::is_concept_or_better_v<std::random_access_iterator_tag, iterator_concept>)
				{
		...
		erase(const_iterator __where) noexcept
		{
			...
			for (; __curr_storage_it != __storage_last; ++__curr_storage_it, (void)++__old_storage_it)
				{
					...
					reference(*__old_storage_it, __ib_detail::__max_binary_index_v<__base_value_type>) = __old_val;
					// shift over
					__curr_pos_ref >>= 1;
				}
			...
			if (_M_bit_pos == 0)
				{
					this->_M_bit_pos = binary_digits_v<__base_value_type> - 1;
					// destroy it
					--__storage_last;
		...
		constexpr void
		swap(bit_sequence& __right) noexcept(::std::is_nothrow_swappable_v<container_type>)
		{
		...
		size() const noexcept
		{
			...
			return (__word_size - 1) * binary_digits_v<__base_value_type> + this->_M_bit_pos;
		}
		...
		operator==(const bit_sequence& __left, const bit_sequence<_RightContainer>& __right)
		{
			using _Left  = bit_sequence;
			using _Right = bit_sequence<_RightContainer>;

			if (__left._M_bit_pos == binary_digits_v<typename _Left::__base_value_type> &&
			     __right._M_bit_pos == binary_digits_v<typename _Right::__base_value_type>)
				{
		...
		operator|=(const bit_sequence<_RightContainer>& __right) noexcept
		{
			using __right_base_t = typename bit_sequence<_RightContainer>::__base_t;
			static_cast<__base_t&>(*this) |= static_cast<const __right_base_t&>(__right);
			return *this;
		}
		...
		constexpr bit_sequence&
		operator<<=(size_type __count) noexcept
		{
			static_cast<__base_t&>(*this) <<= __count;
			return *this;
		}
		...
		template<typename _RightContainer>
		constexpr auto
		operator|(const bit_sequence<_RightContainer>& __right) const noexcept
		{
			auto __result = *this;
			__result |= __right;
			return __result;
		...
		constexpr bit_sequence
		operator<<(size_type __count) const noexcept
		{
			auto __result = *this;
			__result <<= __count;
			return __result;
	...
	private:
		...
		bit_sequence(__ib_detail::__dummy_tag, ::std::pair<container_type, size_type> __container_position) noexcept(
		...
		_S_efficient_empty_assign_into(_Iterator __first, _Sentinel __last, __range& __storage, size_type& __bit_pos)
		{
			// welp...
			...
			for (; __first != __last; ++__first, (void)++__bit_counter, (void)++__modulo_bit_counter, (void)++__it)
				{
					if (__modulo_bit_counter == binary_digits_v<__base_value_type>)
						{
							__base_iterator __storage_it =
		...
		_S_efficient_empty_assign_into_sized(
		     size_type __desired_count, value_type __val, __range& __storage, size_type& __bit_pos)
		{
			size_type __word_insertion = (__desired_count / binary_digits_v<__base_value_type>);
			__bit_pos                  = (__desired_count % binary_digits_v<__base_value_type>);
			if (__bit_pos == 0)
				{
					__bit_pos = binary_digits_v<__base_value_type>;
				}
			else
				{
					++__word_insertion;
		...
		_S_efficient_empty_create(_Iterator __first, _Sentinel __last)
		{
			if (__first == __last)
				{
					return { container_type(), static_cast<size_type>(binary_digits_v<__base_value_type>) };
				}

			if constexpr (::ztd::ranges::is_concept_or_better_v<std::random_access_iterator_tag,
			                   ::ztd::ranges::iterator_concept_t<_Iterator>>)
				{
					size_type __desired_count  = ::std::distance(__first, __last);
					size_type __word_insertion = (__desired_count / binary_digits_v<__base_value_type>);
					size_type __bit_pos        = (__desired_count % binary_digits_v<__base_value_type>);
					if (__bit_pos == 0)
						{
							__bit_pos = binary_digits_v<__base_value_type>;
						}
		     ...
		     difference_type __insert_bit_count, const_iterator __where, _Iterator __first, _Sentinel __last)
		{
			...
			if (__where_pos == 0)
				{
					...
					__base_c_iterator __where_base      = ::std::move(__where).base();
					difference_type __insert_word_count = __insert_bit_count / binary_digits_v<__base_value_type>;
					difference_type __insert_bit_shift  = __insert_bit_count % binary_digits_v<__base_value_type>;
			...
			difference_type __bits_to_boundary = binary_digits_v<__base_value_type> - __where_pos;
			...
			difference_type __leftover_words      = __leftover_bits / binary_digits_v<__base_value_type>;
			difference_type __leftover_words_bits = __leftover_words * binary_digits_v<__base_value_type>;
		     ...
		     __base_c_iterator __where_base, _Iterator& __first)
		{
			if (__insert_word_count > 0)
				{
					difference_type __where_base_dist = __where_dist / binary_digits_v<__base_value_type>;
					difference_type __insert_word_bit_count =
					     __insert_word_count * binary_digits_v<__base_value_type>;
		...
		_M_basic_insert(const_iterator __where, _Iterator __first, _Sentinel __last)
		{
			...
			difference_type __where_position = static_cast<difference_type>(__where - this->cbegin());
			if constexpr (::ztd::ranges::is_concept_or_better_v<::std::bidirectional_iterator_tag,
			                   iterator_concept> &&
			              ::ztd::weakly_decrementable_v<_Sentinel>)
				{
					...
					return ::std::next(this->begin(), __where_position);
				}
			else
				{
					if (__where == this->cend())
		     ...
		     difference_type __erase_word_count) noexcept
		{
			...
			__msb_word_chunk_ref =
			     __ib_detail::__merge_bits_at(__lsb_word_chunk, __msb_word_chunk, __first_position);
			return __storage_it;
		}
		...
		_M_this_random_access_erase(const_iterator __first, const_sentinel __last) noexcept
		{
			...
			difference_type __erase_word_count = __erase_bit_count / binary_digits_v<__base_value_type>;
			difference_type __erase_bit_shift  = __erase_bit_count % binary_digits_v<__base_value_type>;
			...
			for (; __storage_it != __storage_last; ++__storage_it, (void)++__old_storage_it)
				{
					...
					__old_storage_ref = __ib_detail::__replace_most_significant_bits_from<__base_value_type>(
			...
			if (this->_M_bit_pos > static_cast<size_type>(__erase_bit_shift))
			...
			else
				{
					...
					this->_M_bit_pos = binary_digits_v<__base_value_type> - (__erase_bit_shift - this->_M_bit_pos);
				}
			return iterator(::std::move(__first_storage_it), __first_position);
		}
		...
		{
			const __range& __const_storage = ::ztd::unwrap(this->_M_storage);
			return __const_storage;
		}
