...
#define ITSY_BITSY_ALGORITHM_HPP 1
...
namespace bitsy
{
	...
	bit_find_value(bit_iterator<_It> __first, bit_iterator<_It> __last)
	{
		using __iterator       = bit_iterator<_It>;
		using __base_iterator  = typename bit_iterator<_It>::iterator_type;
		...
		for (; __first_base != __last_base; ++__first_base)
			{
				...
				if constexpr (_Value)
					{
						int __first_lsb = bitsy::firstr_one(__ref_base);
						if (__first_lsb != 0)
							{
					...
					{
						int __first_lsb = bitsy::firstr_zero(__ref_base);
						if (__first_lsb != 0)
							{
								return __iterator(::std::move(__first_base), __first_lsb - 1);
							}
	...
	bit_find(bit_iterator<_It> __first, bit_iterator<_It> __last, const _Type& __val)
	{
		...
		if (__val)
			{
				return bit_find_value<true>(::std::move(__first), ::std::move(__last));
			}
		else
			{
				return bit_find_value<false>(::std::move(__first), ::std::move(__last));
			}
	}

	template<typename _InputIt, typename _ForwardIt>
	constexpr bit_iterator<_InputIt>
	bit_find_first_of(bit_iterator<_InputIt> __first, bit_iterator<_InputIt> __last, _ForwardIt __search_first,
	...
	{
		using __iterator0         = bit_iterator<_It0>;
		using __iterator1         = bit_iterator<_It1>;
		using __iterator_concept0 = typename __iterator0::iterator_concept;
		using __iterator_concept1 = typename __iterator1::iterator_concept;
		...
		if constexpr (::std::is_unsigned_v<__base_value_type0> && ::std::is_unsigned_v<__base_value_type1> &&
			...
			{
				if constexpr (::ztd::ranges::is_concept_or_better_v<::std::forward_iterator_tag,
				                   __iterator_concept0> &&
				              ::ztd::ranges::is_concept_or_better_v<::std::forward_iterator_tag,
				                   __iterator_concept1>)
					{
	     ...
	     bit_iterator<_It1> __last1)
	{
		...
		using __base_value_type0  = typename ::std::iterator_traits<__base_iterator0>::value_type;
		using __base_value_type1  = typename ::std::iterator_traits<__base_iterator1>::value_type;
		if constexpr (::ztd::ranges::is_concept_or_better_v<::std::random_access_iterator_tag, __iterator_concept0> &&
		              ::ztd::ranges::is_concept_or_better_v<::std::random_access_iterator_tag, __iterator_concept1>)
			{
	...
	{
		using __iterator0         = bit_iterator<_InputIt0>;
		using __iterator1         = bit_iterator<_InputIt1>;
		using __reference0        = ::ztd::ranges::iterator_reference_t<__iterator0>;
		using __reference1        = ::ztd::ranges::iterator_reference_t<__iterator1>;
		using __iterator_concept0 = ::ztd::ranges::iterator_concept_t<__iterator0>;
		using __iterator_concept1 = ::ztd::ranges::iterator_concept_t<__iterator1>;
		using __base_iterator0    = typename __iterator0::iterator_type;
		using __base_iterator1    = typename __iterator1::iterator_type;
		using __base_value_type0  = ::ztd::ranges::iterator_value_type_t<__base_iterator0>;
		using __base_value_type1  = ::ztd::ranges::iterator_value_type_t<__base_iterator1>;
		if constexpr (::std::is_unsigned_v<__base_value_type0> && ::std::is_unsigned_v<__base_value_type1> &&
		              ::std::is_same_v<__base_value_type0, __base_value_type1>)
			{
				if constexpr (::ztd::ranges::is_concept_or_better_v<::std::forward_iterator_tag,
	...
	constexpr bool
	bit_is_sorted_until_single_bit_position(
	     bit_iterator<_ForwardIt>& __first, bit_iterator<_ForwardIt>& __it, bit_iterator<_ForwardIt>& __last)
	{
	...
	bit_is_sorted_until_unsigned(bit_iterator<_ForwardIt> __first, bit_iterator<_ForwardIt> __last)
	{
		if (__first == __last)
			{
				return __last;
		...
		using __iterator      = bit_iterator<_ForwardIt>;
		...
		if (bit_is_sorted_until_single_bit_position<true>(__first, __it, __last))
			{
				return __it;
			}
		auto __last_position        = __last.position();
		...
		if (__it_base != __last_base)
			{
				...
				using __value_type       = typename bit_iterator<_ForwardIt>::value_type;
				using __base_reference   = ::ztd::ranges::iterator_reference_t<__base_iterator>;
						...
						for (; __it_base != __last_base; ++__it_base)
							{
								...
								::std::size_t __first_lsb_bit = bitsy::firstr_one(__ref_base) - 1;
								const int __all_set_bits      = bitsy::popcount(__ref_base);
								const int __expected_set_bits =
								     static_cast<int>(binary_digits_v<__base_value_type> - __first_lsb_bit);
								...
								for (; __first_lsb_bit < binary_digits_v<__base_value_type>;
				...
				while (__it_base != __last_base)
					{
						__base_reference __ref_base = *__it_base;
						const int __first_zero      = bitsy::firstr_zero(__ref_base);
						if (__first_zero != 0)
							{
	...
	bit_is_sorted_until(bit_iterator<_ForwardIt> __first, bit_iterator<_ForwardIt> __last)
	{
		...
		if constexpr (::std::is_unsigned_v<__base_value_type>)
			{
				return bit_is_sorted_until_unsigned(::std::move(__first), ::std::move(__last));
			}
		else
			{
	...
	{
		return bit_is_sorted_until(__first, __last) == __last;
	}

	template<bool _Value, typename _InputIt>
	constexpr typename ::std::iterator_traits<bit_iterator<_InputIt>>::difference_type
	bit_count_value(bit_iterator<_InputIt> __first, bit_iterator<_InputIt> __last)
	{
		using __iterator        = bit_iterator<_InputIt>;
		using __base_iterator   = typename __iterator::iterator_type;
		using __base_value_type = ::ztd::ranges::iterator_value_type_t<__base_iterator>;
		...
		while (__first_base != __last_base)
			{
				if constexpr (_Value)
					{
						__count += bitsy::popcount(*__first_base);
					}
				else
					{
						__count += (binary_digits_v<__base_value_type> - bitsy::popcount(*__first_base));
					}
				++__first_base;
			}
		if (__last_position == 0)
	...
	constexpr typename ::ztd::ranges::iterator_difference_type_t<bit_iterator<_InputIt>>
	bit_count(bit_iterator<_InputIt> __first, bit_iterator<_InputIt> __last, const _Type& __value)
	{
		if (static_cast<bool>(__value))
			{
				return bit_count_value<true>(::std::move(__first), ::std::move(__last));
			}
		else
			{
				return bit_count_value<false>(::std::move(__first), ::std::move(__last));
			}
	}

	template<typename _It, typename _OutputIt>
	constexpr bit_iterator<_OutputIt>
	bit_copy_same(bit_iterator<_It> __first, bit_iterator<_It> __last, bit_iterator<_OutputIt> __out_first)
	{
		using __iterator          = bit_iterator<_It>;
		using __out_iterator      = bit_iterator<_OutputIt>;
	...
	bit_copy(bit_iterator<_It> __first, bit_iterator<_It> __last, _OutputIt __out_first)
	{
		...
		if constexpr (__ib_detail::__is_bit_iterator_v<_OutputIt>)
			{
				using __base_iterator       = typename bit_iterator<_It>::iterator_type;
	...
	constexpr bit_iterator<_OutputIt>
	bit_copy_n_same(bit_iterator<_It> __first, _Size __count, bit_iterator<_OutputIt> __out_first)
	{
		using __iterator          = bit_iterator<_It>;
		using __base_iterator     = typename __iterator::iterator_type;
		...
		_Size __n_base                    = __count / binary_digits_v<__base_value_type>;
		__out_base_iterator __out_it_base = ::std::copy_n(__first.base(), __n_base, ::std::move(__out_first).base());
		__out_iterator __out_it(::std::move(__out_it_base), 0);
		_Size __last_pos = __count % binary_digits_v<__base_value_type>;
		if (__last_pos == 0)
			{
				return __out_it;
			}
	...
	constexpr bit_iterator<_ForwardIt>
	bit_fill_value(bit_iterator<_ForwardIt> __first, bit_iterator<_ForwardIt> __last)
	{
		using __iterator                   = bit_iterator<_ForwardIt>;
		using __base_iterator              = typename __iterator::iterator_type;
		using __base_value_type            = typename ::std::iterator_traits<__base_iterator>::value_type;
		using __base_underlying_value_type = ::ztd::any_to_underlying_t<__base_value_type>;
	...
	bit_fill(bit_iterator<_ForwardIt> __first, bit_iterator<_ForwardIt> __last, const _Type& __value)
	{
		if (static_cast<bool>(__value))
			{
				return bit_fill_value<true>(::std::move(__first), ::std::move(__last));
			}
		else
			{
				return bit_fill_value<false>(::std::move(__first), ::std::move(__last));
			}
	}

	template<bool _Value, typename _ForwardIt, typename _Size>
	constexpr bit_iterator<_ForwardIt>
	bit_fill_n_value(bit_iterator<_ForwardIt> __first, _Size __count)
	{
		using __iterator                   = bit_iterator<_ForwardIt>;
		...
		__base_iterator __first_base = ::std::move(__first).base();
		for (; __count >= static_cast<__difference_type>(binary_digits_v<__base_value_type>);
		     ++__first_base, (void)(__count -= binary_digits_v<__base_value_type>))
			{
				if constexpr (_Value)
					{
						*__first_base = static_cast<__base_value_type>(
	...
	bit_fill_n(bit_iterator<_OutputIt> __first, _Size __count, const _Type& __value)
	{
		if (static_cast<bool>(__value))
			{
				return bit_fill_n_value<true>(::std::move(__first), ::std::move(__count));
			}
		else
			{
				return bit_fill_n_value<false>(::std::move(__first), ::std::move(__count));
			}
	}
	...
	bit_sort(bit_iterator<_RandomAccessIt> __first, bit_iterator<_RandomAccessIt> __last)
	{
		...
		auto __ones    = bit_count_value<true>(__first, ::std::move(__last));
		auto __zeroes  = __maxbits - __ones;
		auto __mid_it  = bit_fill_n_value<false>(::std::move(__first), __zeroes);
		(void)bit_fill_n_value<true>(::std::move(__mid_it), __ones);
	}
} // namespace bitsy
