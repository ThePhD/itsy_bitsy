     ...
     char32_t, char16_t, char, unsigned char, signed char, std::size_t, std::ptrdiff_t)
{
	...
	const size_type expected_multi_zeroes_count =
	     multi_word_view.size() - expected_multi_ones_count;
	...
	const bit_view_dynamic_bound single_word_zeroes_view(
	     bitsy::dynamic_bit_bounds(0, expected_single_zeroes_count), &single_value,
	     &single_value + 1);
	...
	SECTION("single")
	{
		...
		{
			auto find_false =
			     bitsy::bit_find(single_word_view.cbegin(), single_word_view.cend(), false);
			...
			auto find_true =
			     bitsy::bit_find(single_word_view.cbegin(), single_word_view.cend(), true);
		...
		{
			auto find_false           = bitsy::bit_find_first_of(single_word_view.cbegin(),
                    single_word_view.cend(), il_false.begin(), il_false.end());
			auto find_false_ref       = *find_false;
			size_type find_false_diff = std::distance(single_word_view.cbegin(), find_false);
			auto expected_find_false =
			     std::next(single_word_view.cbegin(), expected_single_lsb_zero_bit_index);
			...
			auto find_true           = bitsy::bit_find_first_of(single_word_view.cbegin(),
                    single_word_view.cend(), il_true.begin(), il_true.end());
			auto find_true_ref       = *find_true;
			size_type find_true_diff = std::distance(single_word_view.cbegin(), find_true);
			auto expected_find_true =
			     std::next(single_word_view.cbegin(), expected_single_lsb_one_bit_index);
			...
			auto find_all           = bitsy::bit_find_first_of(single_word_view.cbegin(),
		...
		SECTION("count")
		{
			...
			size_type count_zeroes_true = bitsy::bit_count(
			     single_word_zeroes_view.cbegin(), single_word_zeroes_view.cend(), true);
			size_type count_zeroes_false = bitsy::bit_count(
			     single_word_zeroes_view.cbegin(), single_word_zeroes_view.cend(), false);
			size_type count_ones_true = bitsy::bit_count(
			     single_word_ones_view.cbegin(), single_word_ones_view.cend(), true);
			size_type count_ones_false = bitsy::bit_count(
			     single_word_ones_view.cbegin(), single_word_ones_view.cend(), false);
		...
		{
			bool is_sorted =
			     bitsy::bit_is_sorted(single_word_view.cbegin(), single_word_view.cend());
			REQUIRE(is_sorted);

			bool is_sorted_ones = bitsy::bit_is_sorted(
			...
			size_type is_sorted_until_diff =
			     std::distance(single_word_view.cbegin(), is_sorted_until);
			REQUIRE(is_sorted_until == single_word_view.cend());
			REQUIRE(is_sorted_until_diff == single_word_view.size());

			auto is_sorted_until_ones = bitsy::bit_is_sorted_until(
			     single_word_ones_view.cbegin(), single_word_ones_view.cend());
		...
		SECTION("equal-4")
		{
			...
			bool equal_truncated =
			     bitsy::bit_equal(single_word_view.cbegin(), single_word_view.cend(),
			          single_word_view.cbegin(), std::next(single_word_view.cbegin(), 4));
			REQUIRE_FALSE(equal_truncated);

			bool equal_first_skewed =
			     bitsy::bit_equal(single_word_ones_view.cbegin(), single_word_ones_view.cend(),
			          single_word_view.cbegin(), single_word_view.cend());
			REQUIRE_FALSE(equal_first_skewed);
		...
		SECTION("lexicograhic_compare")
		{
			bool lexicocgraphic_compare = bitsy::bit_lexicographical_compare(
			     single_word_view.cbegin(), single_word_view.cend(), single_word_view.cbegin(),
			     single_word_view.cend());
			REQUIRE_FALSE(lexicocgraphic_compare);

			bool lexicocgraphic_compare_first_ones = bitsy::bit_lexicographical_compare(
			     single_word_ones_view.cbegin(), single_word_ones_view.cend(),
			     single_word_view.cbegin(), single_word_view.cend());
			REQUIRE_FALSE(lexicocgraphic_compare_first_ones);

			bool lexicocgraphic_compare_second_ones = bitsy::bit_lexicographical_compare(
			     single_word_view.cbegin(), single_word_view.cend(),
			     single_word_ones_view.cbegin(), single_word_ones_view.cend());
	...
	SECTION("multi")
	{
		...
		{
			auto find_false =
			     bitsy::bit_find(multi_word_view.cbegin(), multi_word_view.cend(), false);
			auto find_false_ref       = *find_false;
			...
			auto find_true =
			     bitsy::bit_find(multi_word_view.cbegin(), multi_word_view.cend(), true);
			auto find_true_ref       = *find_true;
		...
		SECTION("find_first_of")
		{
			auto find_false           = bitsy::bit_find_first_of(multi_word_view.cbegin(),
                    multi_word_view.cend(), il_false.begin(), il_false.end());
			auto find_false_ref       = *find_false;
			size_type find_false_diff = std::distance(multi_word_view.cbegin(), find_false);
			auto expected_find_false =
			     std::next(multi_word_view.cbegin(), expected_multi_lsb_zero_bit_index);
			...
			auto find_true           = bitsy::bit_find_first_of(multi_word_view.cbegin(),
                    multi_word_view.cend(), il_true.begin(), il_true.end());
			auto find_true_ref       = *find_true;
			size_type find_true_diff = std::distance(multi_word_view.cbegin(), find_true);
			auto expected_find_true =
			     std::next(multi_word_view.cbegin(), expected_multi_lsb_one_bit_index);
			...
			auto find_all           = bitsy::bit_find_first_of(multi_word_view.cbegin(),
                    multi_word_view.cend(), il_true_false.begin(), il_true_false.end());
			auto find_all_ref       = *find_all;
		...
		SECTION("count")
		{
			...
			size_type count_zeroes_true = bitsy::bit_count(
			     multi_word_zeroes_view.cbegin(), multi_word_zeroes_view.cend(), true);
			size_type count_zeroes_false = bitsy::bit_count(
			     multi_word_zeroes_view.cbegin(), multi_word_zeroes_view.cend(), false);
			size_type count_ones_true = bitsy::bit_count(
			     multi_word_ones_view.cbegin(), multi_word_ones_view.cend(), true);
			size_type count_ones_false = bitsy::bit_count(
			     multi_word_ones_view.cbegin(), multi_word_ones_view.cend(), false);
		...
		{
			bool is_sorted =
			     bitsy::bit_is_sorted(multi_word_view.cbegin(), multi_word_view.cend());
			...
			bool is_sorted_zeroes = bitsy::bit_is_sorted(
			     multi_word_zeroes_view.cbegin(), multi_word_zeroes_view.cend());
			REQUIRE(is_sorted_zeroes);

			auto is_sorted_until =
			     bitsy::bit_is_sorted_until(multi_word_view.cbegin(), multi_word_view.cend());
			size_type is_sorted_until_diff =
			     std::distance(multi_word_view.cbegin(), is_sorted_until);
		...
		SECTION("equal-3")
		{
			...
			bool equal_truncated = bitsy::bit_equal(multi_word_view.cbegin(),
			     std::next(multi_word_view.cbegin(), 4), multi_word_view.cbegin());
			REQUIRE(equal_truncated);
			...
			bool equal_first_skewed = bitsy::bit_equal(multi_word_ones_view.cbegin(),
			     multi_word_ones_view.cend(), multi_word_view.cbegin());
			REQUIRE_FALSE(equal_first_skewed);

			bool equal_second_skewed = bitsy::bit_equal(multi_word_view.cbegin(),
			     std::next(multi_word_view.cbegin(), 4),
			     std::next(multi_word_view.cbegin(), expected_multi_lsb_one_bit_index));
			REQUIRE_FALSE(equal_second_skewed);
		}
		SECTION("equal-4")
		{
			...
			bool equal_truncated =
			     bitsy::bit_equal(multi_word_view.cbegin(), multi_word_view.cend(),
		...
		SECTION("lexicograhic_compare")
		{
			bool lexicocgraphic_compare =
			     bitsy::bit_lexicographical_compare(multi_word_view.cbegin(),
			...
			bool lexicocgraphic_compare_second_ones = bitsy::bit_lexicographical_compare(
			     multi_word_view.cbegin(), multi_word_view.cend(), multi_word_ones_view.cbegin(),
			     multi_word_ones_view.cend());
			REQUIRE(lexicocgraphic_compare_second_ones);
		}
	}
}
