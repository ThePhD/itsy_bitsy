...
TEMPLATE_TEST_CASE("bit algorithm, const", "[algorithm][const]", std::uint64_t, std::uint32_t, std::uint16_t,
     std::uint8_t, std::byte, std::int64_t, std::int32_t, std::int16_t, std::int8_t, char32_t, char16_t, char,
     unsigned char, signed char, std::size_t, std::ptrdiff_t)
{
	...
	const size_type expected_single_ones_count = bitsy::binary_digits_v<TestType> - expected_single_zeroes_count;
	...
	const size_type expected_multi_lsb_one_bit       = (multi_value.size() - 1) * bitsy::binary_digits_v<TestType> + 2;
	...
	const bit_view_dynamic_bound single_word_ones_view(
	     bitsy::dynamic_bit_bounds(
	          expected_single_lsb_one_bit_index, expected_single_lsb_one_bit_index + expected_single_ones_count),
	     &single_value, &single_value + 1);
	const bit_view_dynamic_bound multi_word_zeroes_view(bitsy::dynamic_bit_bounds(0, expected_multi_lsb_one_bit_index),
	     multi_value.data(), multi_value.data() + multi_value.size());
	const bit_view_dynamic_bound multi_word_ones_view(
	     bitsy::dynamic_bit_bounds(expected_multi_lsb_one_bit_index, expected_multi_lsb_one_bit_index + 2),
	     multi_value.data(), multi_value.data() + multi_value.size());
	...
	SECTION("single")
	{
		SECTION("find")
		{
			...
			auto expected_find_false  = std::next(single_word_view.cbegin(), expected_single_lsb_zero_bit_index);
			...
			auto expected_find_true  = std::next(single_word_view.cbegin(), expected_single_lsb_one_bit_index);
		...
		SECTION("find_first_of")
		{
			...
			auto find_all = bitsy::bit_find_first_of(
			     single_word_view.cbegin(), single_word_view.cend(), il_true_false.begin(), il_true_false.end());
			auto find_all_ref       = *find_all;
			size_type find_all_diff = std::distance(single_word_view.cbegin(), find_all);
			auto expected_find_all  = std::next(single_word_view.cbegin(), expected_single_lsb_zero_bit_index);
		...
		SECTION("count")
		{
			size_type count_true  = bitsy::bit_count(single_word_view.cbegin(), single_word_view.cend(), true);
			size_type count_false = bitsy::bit_count(single_word_view.cbegin(), single_word_view.cend(), false);
		...
		SECTION("is_sorted")
		{
			...
			bool is_sorted_ones = bitsy::bit_is_sorted(single_word_ones_view.cbegin(), single_word_ones_view.cend());
			REQUIRE(is_sorted_ones);

			auto is_sorted_until = bitsy::bit_is_sorted_until(single_word_view.cbegin(), single_word_view.cend());
		...
		{
			bool equal =
			     bitsy::bit_equal(single_word_view.cbegin(), single_word_view.cend(), single_word_view.cbegin());
			REQUIRE(equal);

			bool equal_truncated = bitsy::bit_equal(
			     single_word_view.cbegin(), std::next(single_word_view.cbegin(), 4), single_word_view.cbegin());
			REQUIRE(equal_truncated);

			bool equal_empty =
			     bitsy::bit_equal(single_word_view.cbegin(), single_word_view.cbegin(), single_word_view.cend());
			REQUIRE(equal_empty);
		...
		SECTION("equal-4")
		{
			...
			bool equal_first_skewed = bitsy::bit_equal(single_word_ones_view.cbegin(), single_word_ones_view.cend(),
			     single_word_view.cbegin(), single_word_view.cend());
			REQUIRE_FALSE(equal_first_skewed);

			bool equal_second_skewed = bitsy::bit_equal(single_word_view.cbegin(), single_word_view.cend(),
			     single_word_ones_view.cbegin(), single_word_ones_view.cend());
			REQUIRE_FALSE(equal_second_skewed);

			bool equal_first_range_empty = bitsy::bit_equal(single_word_view.cbegin(), single_word_view.cbegin(),
			     single_word_view.cbegin(), single_word_view.cend());
			REQUIRE_FALSE(equal_first_range_empty);

			bool equal_second_range_empty = bitsy::bit_equal(single_word_view.cbegin(), single_word_view.cend(),
			     single_word_view.cend(), single_word_view.cend());
			REQUIRE_FALSE(equal_second_range_empty);

			bool equal_all_empty = bitsy::bit_equal(single_word_ones_view.cend(), single_word_ones_view.cend(),
			     single_word_view.cend(), single_word_view.cend());
		...
		SECTION("lexicograhic_compare")
		{
			...
			bool lexicocgraphic_compare_first_ones =
			     bitsy::bit_lexicographical_compare(single_word_ones_view.cbegin(), single_word_ones_view.cend(),
			          single_word_view.cbegin(), single_word_view.cend());
			REQUIRE_FALSE(lexicocgraphic_compare_first_ones);

			bool lexicocgraphic_compare_second_ones = bitsy::bit_lexicographical_compare(single_word_view.cbegin(),
	...
	SECTION("multi")
	{
		SECTION("find")
		{
			...
			auto expected_find_false  = std::next(multi_word_view.cbegin(), expected_multi_lsb_zero_bit_index);
			REQUIRE(find_false == expected_find_false);
			...
			auto expected_find_true  = std::next(multi_word_view.cbegin(), expected_multi_lsb_one_bit_index);
			REQUIRE(find_true == expected_find_true);
		...
		SECTION("find_first_of")
		{
			...
			auto find_all = bitsy::bit_find_first_of(
			     multi_word_view.cbegin(), multi_word_view.cend(), il_true_false.begin(), il_true_false.end());
			auto find_all_ref       = *find_all;
			size_type find_all_diff = std::distance(multi_word_view.cbegin(), find_all);
			auto expected_find_all  = std::next(multi_word_view.cbegin(), expected_multi_lsb_zero_bit_index);
			REQUIRE(find_all == expected_find_all);
		...
		SECTION("count")
		{
			size_type count_true  = bitsy::bit_count(multi_word_view.cbegin(), multi_word_view.cend(), true);
			size_type count_false = bitsy::bit_count(multi_word_view.cbegin(), multi_word_view.cend(), false);
			size_type count_zeroes_true =
		...
		SECTION("is_sorted")
		{
			...
			bool is_sorted_ones = bitsy::bit_is_sorted(multi_word_ones_view.cbegin(), multi_word_ones_view.cend());
			REQUIRE(is_sorted_ones);
			...
			auto is_sorted_until = bitsy::bit_is_sorted_until(multi_word_view.cbegin(), multi_word_view.cend());
			...
			size_type expected_is_sorted_until_diff = multi_word_zeroes_view.size() + multi_word_ones_view.size();
			...
			size_type is_sorted_until_ones_diff = std::distance(multi_word_ones_view.cbegin(), is_sorted_until_ones);
		...
		{
			bool equal =
			     bitsy::bit_equal(multi_word_view.cbegin(), multi_word_view.cend(), multi_word_view.cbegin());
			REQUIRE(equal);
			...
			bool equal_empty =
			     bitsy::bit_equal(multi_word_view.cbegin(), multi_word_view.cbegin(), multi_word_view.cend());
			REQUIRE(equal_empty);
		...
		SECTION("equal-4")
		{
			bool equal = bitsy::bit_equal(
			     multi_word_view.cbegin(), multi_word_view.cend(), multi_word_view.cbegin(), multi_word_view.cend());
			REQUIRE(equal);
			...
			bool equal_first_skewed = bitsy::bit_equal(multi_word_ones_view.cbegin(), multi_word_ones_view.cend(),
			...
			bool equal_first_range_empty = bitsy::bit_equal(multi_word_view.cbegin(), multi_word_view.cbegin(),
			     multi_word_view.cbegin(), multi_word_view.cend());
			REQUIRE_FALSE(equal_first_range_empty);

			bool equal_second_range_empty = bitsy::bit_equal(
			     multi_word_view.cbegin(), multi_word_view.cend(), multi_word_view.cend(), multi_word_view.cend());
			REQUIRE_FALSE(equal_second_range_empty);

			bool equal_all_empty = bitsy::bit_equal(multi_word_ones_view.cend(), multi_word_ones_view.cend(),
			     multi_word_view.cend(), multi_word_view.cend());
			REQUIRE(equal_all_empty);
		}
		SECTION("lexicograhic_compare")
		{
			bool lexicocgraphic_compare = bitsy::bit_lexicographical_compare(
			     multi_word_view.cbegin(), multi_word_view.cend(), multi_word_view.cbegin(), multi_word_view.cend());
			REQUIRE_FALSE(lexicocgraphic_compare);

			bool lexicocgraphic_compare_first_ones =
			     bitsy::bit_lexicographical_compare(multi_word_ones_view.cbegin(), multi_word_ones_view.cend(),
			          multi_word_view.cbegin(), multi_word_view.cend());
			REQUIRE_FALSE(lexicocgraphic_compare_first_ones);

			bool lexicocgraphic_compare_second_ones = bitsy::bit_lexicographical_compare(multi_word_view.cbegin(),
