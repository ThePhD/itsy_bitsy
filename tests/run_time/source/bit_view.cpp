...
TEMPLATE_TEST_CASE("bit_view functionality with ranges", "[bit_view][ranges]", std::size_t,
  ...
  signed char, std::int32_t, std::int64_t)
{
  ...
  {
    auto storage_test = [&](auto& storage) {
      using R = ranges::subrange<decltype(std::cbegin(storage)), decltype(std::cend(storage))>;
      R view(std::cbegin(storage), std::cend(storage));
      bitsy::bit_view<R> view_bits(view);
