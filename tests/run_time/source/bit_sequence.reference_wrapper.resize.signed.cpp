...
TEMPLATE_TEST_CASE("bit_sequence reference_wrapper bulk resize test, signed",
     "[bit_sequence][reference_wrapper][bulk][resize][signed]", std::int64_t, std::int32_t, std::int16_t, std::int8_t,
     char, signed char, std::ptrdiff_t)
{
	SECTION("vector")
	{
		std::vector<TestType> backing_storage;
		{
			...
			bit_sequence_resize_test_bulk_small<TestType>(storage);
		}
	...
	SECTION("list")
	{
		...
		{
			bitsy::bit_sequence<std::reference_wrapper<std::list<TestType>>> storage(std::in_place, backing_storage);
			bit_sequence_resize_test_bulk_small<TestType>(storage);
		}
		REQUIRE(backing_storage.size() > 0);
	}
