...
{
	VERIFY_FALSE(view_bits.none());
	VERIFY_FALSE(view_bits.all());
	VERIFY(view_bits.any());
}
  ...
  std::size_t expected_bits = expected_words * bitsy::binary_digits_v<TestType>)
{
	...
	for (const auto& off_index : off_indices)
		{
			...
			VERIFY_FALSE(off_index_bit);
			VERIFY(off_index_bit == view_bits.test(off_index));
		}
	...
	for (const auto& on_index : on_indices)
		{
			...
			VERIFY(on_index_bit);
			VERIFY(on_index_bit == view_bits.test(on_index));
		}

	{
		...
		VERIFY(iter_count == expected_bits);
		VERIFY(iter_on_count == expected_on_bits);
		VERIFY(iter_off_count == expected_off_bits);
	}

	VERIFY(view_bits.one_count() == expected_on_bits);
	VERIFY(view_bits.count(true) == expected_on_bits);
	VERIFY(view_bits.count(false) == expected_off_bits);
}
  ...
  std::size_t expected_bits = expected_words * bitsy::binary_digits_v<TestType>)
{
	...
	VERIFY(span_bits.one_count() == initial_expected_on_bits);
	VERIFY(span_bits.count(true) == initial_expected_on_bits);
	VERIFY(span_bits.count(false) == initial_expected_off_bits);

	for (const auto& flip_index : on_indices)
		{
			auto flip_index_bit = span_bits[flip_index];
			VERIFY(flip_index_bit);
			VERIFY(flip_index_bit == span_bits.test(flip_index));
			span_bits.flip(flip_index);
			VERIFY_FALSE(flip_index_bit);
			VERIFY(flip_index_bit == span_bits.test(flip_index));
		}

	VERIFY(span_bits.one_count() == post_expected_on_bits);
	VERIFY(span_bits.count(true) == post_expected_on_bits);
	VERIFY(span_bits.count(false) == post_expected_off_bits);
	...
	for (const auto& set_index : on_indices)
		{
			...
			VERIFY(set_index_bit);
			VERIFY(set_index_bit == span_bits.test(set_index));
			span_bits.set(set_index, false);
			VERIFY_FALSE(set_index_bit);
			VERIFY(set_index_bit == span_bits.test(set_index));
		}
	...
	VERIFY(span_bits.popcount() == initial_expected_on_bits);
	VERIFY(span_bits.count(true) == initial_expected_on_bits);
	VERIFY(span_bits.count(false) == initial_expected_off_bits);
}
